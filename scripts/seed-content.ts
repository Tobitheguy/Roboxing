...
const POSTS = [
  ...
  },
  {
    ...
    summary:
      "Every fight this sport has staged had a human at the controls. Unitree's world model removes the operator and puts a person in the ring instead — though the footage suggests the thinking may not be happening on board.",
    ...
    body: [
      ...
      "The opponent is the part worth pausing on: the G1 sparred a HUMAN, not another machine. A robot trading blows with a scripted counterpart is a choreography problem. A robot reading a person — who feints, hesitates, and does the unrehearsed thing — is the harder version of the claim, and it is the one Unitree made.",
      ...
      "One caveat goes to the substance rather than the setting. Console output visible in the footage suggests the model may not be running on the robot at all — sensor data appears to leave for external computers, which return movement commands. Unitree's own framing acknowledges that reducing this reliance on off-board computing remains an open problem. A machine that is autonomous because a desktop nearby is doing the thinking is a meaningfully smaller claim than an autonomous machine, and the difference matters most precisely when a league has to decide what it is sanctioning.",
